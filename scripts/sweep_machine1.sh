#!/bin/bash
# Sweep machine 1: 12 of 24 combinations
...
BASE="CUDA_VISIBLE_DEVICES=0 python main.py --arch simple --cssm add_kqv --dataset pathfinder \
    --pathfinder_difficulty 14 --tfrecord_dir /oscar/scratch/dlinsley/pathfinder_tfrecords_128/ \
    --gate_type factored --embed_dim 32 --seq_len 8 --batch_size 256 --lr 3e-4 --epochs 20 \
    --no_wandb --use_complex32 --stem_layers 1 --pos_embed separate --kernel_size 11 \
    --image_size 128"
...
run_experiment() {
    ...
    local name="sweep_${sno}_${li_tag}_${nt}_${pt}"
    echo "========================================"
    echo "Running: $name"
    echo "========================================"
...
# Machine 1 also gets: stem_norm_order=post, learned_init=0 (6)
# (not needed — already have 12 from pre)

echo "Machine 1 sweep complete."
