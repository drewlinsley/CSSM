#!/bin/bash
# Transformer + GDN sweep on PathTracker 32-frame version
# Same hyperparameters as 64-frame sweep, but with 32-frame videos.
#
...
#
# Usage:
#   bash scripts/sweep_transformer_pathtracker_32f.sh 2>&1 | tee -a sweep_pathtracker_32f_$(hostname).out

set -uo pipefail

# ── Configuration ────────────────────────────────────────────────────────────
PROGRESS_LOG="sweep_pathtracker_32f_progress.log"
touch "$PROGRESS_LOG"

TFRECORD_DIR="/oscar/scratch/dlinsley/pathtracker_equal_large_32f_tfrecords"
PROJECT="CSSM_pathtracker_32f"
...
for DEPTH in 1 3; do
  ...
  for DIM in 32 64; do
    ...
    for DROP_PATH in $DROP_PATHS; do
      ...
      NAME="32f_st_d${DEPTH}_e${DIM}_dp${DP_SHORT}_mlp40"
      ...
      dispatch_run "$NAME" 128 \
          ...
          --frame_readout all --pos_embed mrope --seq_len 32 --max_seq_len 32 --act_type gelu \
          --inter_mlp_ratio 4.0 \
    ...
    for DIM in 32 64; do
      for CONV in 1 5; do
        for DK in 1 2 4; do
          ...
          NAME="32f_gdn_d${DEPTH}_e${DIM}_qkv${CONV}_dk${DK}"
          ...
          dispatch_run "$NAME" 128 \
              ...
              --seq_len 32 --max_seq_len 32 --norm_type ${GDN_NORM} \
              --drop_path_rate 0.0 \
...
# ── Spatiotemporal head sweep (depth=1, lr=1e-3): heads in {1,4,8} -- 5 runs ─
for DIM in 32 64; do
  if [ "$DIM" = "32" ]; then HEAD_LIST="1 4"; else HEAD_LIST="1 2 8"; fi

  for HEADS in $HEAD_LIST; do
    RUN_IDX=$((RUN_IDX + 1))
    NAME="32f_st_d1_e${DIM}_h${HEADS}"
...
for DIM in 32 64; do
  for DK in 1 2 4; do
    ...
    NAME="32f_gdnint_d1_e${DIM}_dk${DK}"

    dispatch_run "$NAME" 128 \
        --arch simple --cssm gdn_int --dataset pathtracker \
        --stem_mode pathtracker --embed_dim ${DIM} --gate_type factored \
        --kernel_size 11 --pool_type max --pos_embed mrope \
        --seq_len 32 --max_seq_len 32 --norm_type ${GDN_NORM} --use_complex32 \
        --drop_path_rate 0.0 \
...
for DIM in 32 64; do
  for DK in 2 4; do
    RUN_IDX=$((RUN_IDX + 1))
    NAME="32f_gdnint_elem_d1_e${DIM}_dk${DK}"
...
for DIM in 32 64; do
  for DK in 2 4; do
    RUN_IDX=$((RUN_IDX + 1))
    NAME="32f_gdnint_qk_d1_e${DIM}_dk${DK}"
...
for DIM in 32 64; do
  for DK in 2 4; do
    RUN_IDX=$((RUN_IDX + 1))
    NAME="32f_gdnseq_d1_e${DIM}_dk${DK}"
...
for DIM in 32 64; do
  for KS in 3 5; do
    RUN_IDX=$((RUN_IDX + 1))
    NAME="32f_convssm_d1_e${DIM}_ks${KS}"
