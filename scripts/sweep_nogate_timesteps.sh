#!/bin/bash
# NoGate CSSM 7×7 timestep sweep on Pathfinder CL-14
#
# Tests whether the spectral kernel alone (no B/C/Delta gates) needs
# temporal recurrence to work. At T=8 it gets 86.1% on CL-14.
...
PROGRESS_LOG="14_sweep_nogate_ts_progress.log"
touch "$PROGRESS_LOG"
...
run_with_retry() {
    ...
    shift 2
    local BS=$START_BS MIN_BS=8 ATTEMPT=0 LOGFILE="/tmp/14_sweep_run_${NAME}.log"
...
COMMON="--dataset pathfinder --pathfinder_difficulty 14 --image_size 128 --stem_layers 1 \
